...
func TestParseLang(t *testing.T) {
    ...
    buffer.WriteString(
        ...
        "\t#indented comment\n" +
        "  \"[0-9]*\" int  ")
    lang, err := ParseLang(bufio.NewScanner(&buffer))
...
func TestParseLangErrorNoInitQuote(t *testing.T) {
    ...
    _, err := ParseLang(bufio.NewScanner(&buffer))
...
func TestLex(t *testing.T) {
    ...
    langBuf.WriteString(
        ...
        "\"[0-9]+\"     int\n" +
        "\"\\pZ*\"      @skip")
    lang, langErr := ParseLang(bufio.NewScanner(&langBuf))
    ...
    go Lex(lang, bufio.NewScanner(&inputBuf), tokStream)
